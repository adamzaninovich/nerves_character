defmodule HelloNerves.Scene.SysInfo do
  use Scenic.Scene
  alias Scenic.Graph
  ...
  @graph Graph.build(font_size: 22, font: :roboto_mono)
         |> group(
           fn g ->
             ...
             |> text("", translate: {10, 74}, font_size: 18, id: :net_info)
           end,
           t: {10, 30}
         )
         |> group(
  ...
  def init(_, opts) do
    ...
    ssid =
      ...
      |> Map.get(:networks, [%{}])
      |> hd()
      |> Map.get(:ssid, "Not Connected to WiFi")
    ...
    graph =
      ...
      |> Graph.modify(:net_info, &text(&1, net_info))
  ...
  unless @target == "host" do
    ...
    def handle_info(:update_devices, graph) do
      ...
      devices =
        Enum.reduce(InputEvent.enumerate(), "", fn {_, device}, acc ->
